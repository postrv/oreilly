...
fn main() {
    ...
    //render(&mut pixels, bounds, upper_left, lower_right); removed to make way for parallel function
    let threads = 8;
...
///
/// If `c` is not a member, return `Some(i)` where `i` is the number
/// of iterations it took for 'c' to leave its origin
///
/// If `c` seems to be a member (more precisely,
/// if we reached the iteration limit without being able
